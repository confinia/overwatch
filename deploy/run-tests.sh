...
set -e
cd /tmp && cp -r /src/api . && cp -r /src/db . && cp -r /src/web . && cp /src/docker-compose.selfhost.yml . && mkdir -p deploy/caddy && cp /src/deploy/caddy/Caddyfile.selfhost deploy/caddy/
...
cd api
python -m pytest test_subscription.py test_rls.py test_frontend.py test_selfhost.py -q 2>&1 | tail -1
