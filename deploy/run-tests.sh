...
set -e
cd /tmp && cp -r /src/api . && cp -r /src/db . && cp -r /src/web . && cp -r /src/grafana . && cp /src/docker-compose.selfhost.yml . && cp /faq/FAQ.md . && mkdir -p deploy/caddy && cp /src/deploy/caddy/Caddyfile.selfhost deploy/caddy/ && cp /dr/backup.sh /dr/restore.sh deploy/
...
cd api
python -m pytest test_subscription.py test_rls.py test_frontend.py test_selfhost.py test_faq.py test_backup.py test_fields.py test_dashboards.py test_spacecraft.py -q 2>&1 | tail -1
